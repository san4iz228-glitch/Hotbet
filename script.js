...
function startMatches() {
    setInterval(()=>{
            ...
            if(!event.settled && now >= event.startTime) {
                ...
                if(elapsed <= event.duration) {
                    ...
                    event.bets.forEach(bet=>{
                        let userData = JSON.parse(localStorage.getItem(bet.user));
        ...
        renderEvents();
        if(currentUser) document.getElementById("balance").textContent = JSON.parse(localStorage.getItem(currentUser)).balance;
    },1000);
}